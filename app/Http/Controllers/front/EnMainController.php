...
class EnMainController extends Controller
{
    public function index()
    {
        $sliders = Slider::latest()->get();
        $home = Product::latest()->get();
        $homeCountries = $home->pluck('encountry');
        $homeCities = $home->pluck('encity');
        $brands = Brand::latest()->get();
        $cars = Car::latest()->get();
        $carcity = $cars->pluck('encity');
        $agents = Cosult::latest()->get();
        $blogs = Blog::where('status',1)->latest()->get();
        return view('front.en.index', compact('carcity','sliders', 'homeCountries', 'homeCities', 'home', 'brands', 'cars','agents','blogs'));
    }
    ...
    public function search(Request $request)
    {
        if ($request->home) {
            ...
            if ($request->country) {
                $product = Product::where('encountry', 'LIKE', '%' . $request->country . '%')->get();
            }
            if ($request->city) {
                $product = Product::where('encity', 'LIKE', '%' . $request->city . '%')->get();
            }
            ...
            $home = 'Home';
            $results = $product;
            return view('front.en.search', compact('results', 'home'));
        }
        elseif ($request->car) {
            ...
            if ($request->city) {
                $car = $car->where('encity',  $request->city );
            }
            if ($request->brand) {
            ...
            $vehicle = 'vehicle';
            $results = $car;
            return view('front.en.search', compact('results', 'vehicle'));
        }
        return back();
    }
    ...
    public function about()
    {
        ...
        return view('front.en.about',compact('products','cars'));
    }

    public function service()
    {
        $agents = Cosult::latest()->get();
        return view('front.en.service',compact('agents'));
    }
    ...
    public function list()
    {
        ...
        return view('front.en.lists', compact('homes', 'homeCountries', 'homeCities'));
    }
    ...
    public function carlist()
    {
        ...
        $brands =Brand::latest()->get();
        return view('front.en.carlist', compact('brands','cars', 'homeCities'));
    }
    ...
    public function product($home, $slug)
    {
        if ($home == "Home") {
            $home = Product::latest()->get();
            $homeCountries = $home->unique('encountry')->pluck('encountry');
            $homeCities = $home->unique('encity')->pluck('encity');
            $product = Product::where('slug', $slug)->first();
            $consult = Cosult::whereId($product->cosult_id)->first();
            $car=null;$cars=null;$brands=null;
        } elseif ($home == "car") {
            $cars = Car::latest()->get();
            $homeCountries = $cars->unique('encountry')->pluck('encountry');
            $homeCities = $cars->unique('encity')->pluck('encity');
        ...
        return view('front.en.product', compact('brands','product', 'properties', 'homeCities', 'homeCountries', 'home','car','cars','consult'));
    }
    ...
    public function blogDetail($id)
    {
        ...
        return view('front.en.blogdetail',compact('blog','blogs'));
    }

    public function searchpage()
    {
        return view('front.en.search');
    }
    ...
    public function agent($name)
    {
        ...
        return view('front.en.agent',compact('agent','cars','homes'));
    }
